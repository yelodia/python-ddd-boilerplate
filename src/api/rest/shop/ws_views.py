...
shop_ws_router = APIRouter(prefix="/ws", tags=["websocket"])


@shop_ws_router.websocket("/products")
async def products_ws(websocket: WebSocket, mgr: WsManagerDep) -> None:
    """
    WebSocket-подписка на обновления витрины.

    Клиент получает push-уведомления при изменении любого товара:  # TODO обновить примеры пакетов, эти уже устарели
...
@shop_ws_router.websocket("/carts/{cart_id}")
async def cart_ws(websocket: WebSocket, cart_id: int, mgr: WsManagerDep) -> None:
    ...
    Клиент получает push-уведомления при изменении состава корзины:  # TODO обновить примеры пакетов, эти уже устарели
